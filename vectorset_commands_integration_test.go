...
func generateRandomVector(dim int) kv.VectorValues {
	rand.Seed(time.Now().UnixNano())
	v := make([]float64, dim)
	for i := range v {
		v[i] = float64(rand.Intn(1000)) + rand.Float64()
	}
	return kv.VectorValues{Val: v}
}

var _ = Describe("KV VectorSet commands", Label("vectorset"), func() {
	ctx := context.TODO()
	...
	for _, protocol := range protocols {
		...
		Context(fmt.Sprintf("with protocol version %d", protocol), func() {
			...
			It("basic", func() {
				SkipBeforeKVVersion(8.0, "KV 8.0 introduces support for VectorSet")
				vecName := "basic"
				val := &kv.VectorValues{
			...
			It("basic similarity", func() {
				...
				ok, err := client.VAdd(ctx, vecName, "k1", &kv.VectorValues{
					Val: []float64{1, 0, 0, 0},
				}).Result()
				expectNil(err)
				expectTrue(ok)
				ok, err = client.VAdd(ctx, vecName, "k2", &kv.VectorValues{
					Val: []float64{0.99, 0.01, 0, 0},
				}).Result()
				expectNil(err)
				expectTrue(ok)
				ok, err = client.VAdd(ctx, vecName, "k3", &kv.VectorValues{
					Val: []float64{0.1, 1, -1, 0.5},
				}).Result()
				expectNil(err)
				expectTrue(ok)

				sim, err := client.VSimWithScores(ctx, vecName, &kv.VectorValues{
					Val: []float64{1, 0, 0, 0},
				}).Result()
				expectNil(err)
			...
			It("dimension operation", func() {
				...
				ok, err := client.VAddWithArgs(ctx, vecName, "k1", &v1, &kv.VAddArgs{
				...
				_, err = client.VAddWithArgs(ctx, vecName, "kw", &wrongV, &kv.VAddArgs{
					Reduce: int64(reducedDim),
				}).Result()
				expectTrue(err != nil)

				v2 := generateRandomVector(originalDim)
				ok, err = client.VAddWithArgs(ctx, vecName, "k2", &v2, &kv.VAddArgs{
					Reduce: int64(reducedDim),
				}).Result()
				expectNil(err)
				expectTrue(ok)
			...
			It("all operations", func() {
				...
				vals := []struct {
					...
					{
						name: "k0",
						v:    kv.VectorValues{Val: []float64{1, 0, 0, 0}},
						attr: `{"age": 25, "name": "Alice", "active": true, "scores": [85, 90, 95], "city": "New York"}`,
					},
					{
						name: "k1",
						v:    kv.VectorValues{Val: []float64{0, 1, 0, 0}},
						attr: `{"age": 30, "name": "Bob", "active": false, "scores": [70, 75, 80], "city": "Boston"}`,
					},
					{
						name: "k2",
						v:    kv.VectorValues{Val: []float64{0, 0, 1, 0}},
						attr: `{"age": 35, "name": "Charlie", "scores": [60, 65, 70], "city": "Seattle"}`,
					},
					{
						name: "k3",
						v:    kv.VectorValues{Val: []float64{0, 0, 0, 1}},
					},
					{
						name: "k4",
						v:    kv.VectorValues{Val: []float64{0.5, 0.5, 0, 0}},
						attr: `invalid json`,
					},
				}
				...
				// test equality
				sim, err := client.VSimWithArgs(ctx, vecName, &vals[0].v, &kv.VSimArgs{
					Filter: `.age == 25`,
				}).Result()
				expectNil(err)
