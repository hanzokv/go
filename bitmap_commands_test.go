...
var _ = Describe("BitCountBite", func() {
	...
	It("bit count bite", func() {
		...
		for _, e := range expected {
			cmd := client.BitCount(ctx, key, &kv.BitCount{Start: e.Start, End: e.End, Unit: kv.BitCountIndexBit})
			Expect(cmd.Err()).NotTo(HaveOccurred())
...
var _ = Describe("BitCountByte", func() {
	...
	It("bit count byte", func() {
		...
		for _, e := range expected {
			cmd := client.BitCount(ctx, key, &kv.BitCount{Start: e.Start, End: e.End, Unit: kv.BitCountIndexByte})
			Expect(cmd.Err()).NotTo(HaveOccurred())
			Expect(cmd.Val()).To(Equal(e.Expected))
		}
	})
	...
	It("bit count byte with no unit specified", func() {
		...
		for _, e := range expected {
			cmd := client.BitCount(ctx, key, &kv.BitCount{Start: e.Start, End: e.End})
			Expect(cmd.Err()).NotTo(HaveOccurred())
			Expect(cmd.Val()).To(Equal(e.Expected))
		}
	})
